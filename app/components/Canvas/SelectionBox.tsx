...
interface SelectionBoxProps {
  ...
  // New props for other modals
  upscaleModalStates: any[];
  setUpscaleModalStates: any;
...
export const SelectionBox: React.FC<SelectionBoxProps> = ({
  ...
  onImageUpdate,
  upscaleModalStates, setUpscaleModalStates, selectedUpscaleModalIds, setSelectedUpscaleModalIds, onPersistUpscaleModalMove,
  removeBgModalStates, setRemoveBgModalStates, selectedRemoveBgModalIds, setSelectedRemoveBgModalIds, onPersistRemoveBgModalMove,
  eraseModalStates, setEraseModalStates, selectedEraseModalIds, setSelectedEraseModalIds, onPersistEraseModalMove,
  expandModalStates, setExpandModalStates, selectedExpandModalIds, setSelectedExpandModalIds, onPersistExpandModalMove,
  vectorizeModalStates, setVectorizeModalStates, selectedVectorizeModalIds, setSelectedVectorizeModalIds, onPersistVectorizeModalMove,
  storyboardModalStates, setStoryboardModalStates, selectedStoryboardModalIds, setSelectedStoryboardModalIds, onPersistStoryboardModalMove,
  scriptFrameModalStates, setScriptFrameModalStates, selectedScriptFrameModalIds, setSelectedScriptFrameModalIds, onScriptFramePositionCommit,
  sceneFrameModalStates, setSceneFrameModalStates, selectedSceneFrameModalIds, setSelectedSceneFrameModalIds, onSceneFramePositionCommit,
}) => {
  ...
  const collectSelectedComponents = (): SelectedComponent[] => {
    ...
    selectedScriptFrameModalIds.forEach((id) => {
      ...
      components.push({
        ...
        width: modal.frameWidth,
        height: modal.frameHeight,
        x: modal.x,
        y: modal.y,
      });
    });
  ...
  // 1. There's a selection rect AND it's a drag selection with 1+ components, OR
  // 2. There's a selection rect AND a group is selected (for group dragging and ungroup button)
  if (selectionTightRect && isDragSelection && totalSelected >= 1) {
      ...
      <>
        <Group
          ...
          onDragStart={(e) => {
            ...
            // Store original removeBg modal positions
            ...
            selectedStoryboardModalIds.forEach(modalId => {
              ...
              if (modalState) {
                originalPositions.storyboardModals.set(modalId, { x: modalState.x ?? 0, y: modalState.y ?? 0 });
              }
            });

            // Store original scriptFrame modal positions
            ...
            // Store original sceneFrame modal positions
          ...
          onDragMove={(e) => {
            ...
            // Move all selected upscale modals
            selectedUpscaleModalIds.forEach(modalId => {
              const originalPos = originalPositions.upscaleModals.get(modalId);
              if (originalPos) {
                setUpscaleModalStates((prev: any[]) => prev.map((m: any) => m.id === modalId ? { ...m, x: originalPos.x + deltaX, y: originalPos.y + deltaY } : m));
              }
            });

            // Move all selected removeBg modals
            selectedRemoveBgModalIds.forEach(modalId => {
              const originalPos = originalPositions.removeBgModals.get(modalId);
              if (originalPos) {
                setRemoveBgModalStates((prev: any[]) => prev.map((m: any) => m.id === modalId ? { ...m, x: originalPos.x + deltaX, y: originalPos.y + deltaY } : m));
              }
            });

            // Move all selected erase modals
            selectedEraseModalIds.forEach(modalId => {
              const originalPos = originalPositions.eraseModals.get(modalId);
              if (originalPos) {
                setEraseModalStates((prev: any[]) => prev.map((m: any) => m.id === modalId ? { ...m, x: originalPos.x + deltaX, y: originalPos.y + deltaY } : m));
              }
            });

            // Move all selected expand modals
            selectedExpandModalIds.forEach(modalId => {
              const originalPos = originalPositions.expandModals.get(modalId);
              if (originalPos) {
                setExpandModalStates((prev: any[]) => prev.map((m: any) => m.id === modalId ? { ...m, x: originalPos.x + deltaX, y: originalPos.y + deltaY } : m));
              }
            });

            // Move all selected vectorize modals
            selectedVectorizeModalIds.forEach(modalId => {
              const originalPos = originalPositions.vectorizeModals.get(modalId);
              if (originalPos) {
                setVectorizeModalStates((prev: any[]) => prev.map((m: any) => m.id === modalId ? { ...m, x: originalPos.x + deltaX, y: originalPos.y + deltaY } : m));
              }
            });

            // Move all selected storyboard modals
            selectedStoryboardModalIds.forEach(modalId => {
              const originalPos = originalPositions.storyboardModals.get(modalId);
              if (originalPos) {
                setStoryboardModalStates((prev: any[]) => prev.map((m: any) => m.id === modalId ? { ...m, x: originalPos.x + deltaX, y: originalPos.y + deltaY } : m));
              }
            });

            // Move all selected scriptFrame modals
            selectedScriptFrameModalIds.forEach(modalId => {
              const originalPos = originalPositions.scriptFrameModals.get(modalId);
              if (originalPos) {
                setScriptFrameModalStates((prev: any[]) => prev.map((m: any) => m.id === modalId ? { ...m, x: originalPos.x + deltaX, y: originalPos.y + deltaY } : m));
              }
            });

            // Move all selected sceneFrame modals
            selectedSceneFrameModalIds.forEach(modalId => {
              const originalPos = originalPositions.sceneFrameModals.get(modalId);
              if (originalPos) {
                setSceneFrameModalStates((prev: any[]) => prev.map((m: any) => m.id === modalId ? { ...m, x: originalPos.x + deltaX, y: originalPos.y + deltaY } : m));
              }
            });
          ...
          onDragEnd={(e) => {
            ...
            if (onPersistUpscaleModalMove) {
              selectedUpscaleModalIds.forEach(modalId => {
                const modalState = upscaleModalStates.find(m => m.id === modalId);
                if (modalState) Promise.resolve(onPersistUpscaleModalMove(modalId, { x: modalState.x, y: modalState.y })).catch(console.error);
              });
            }

            // Persist final positions for removeBg modals
            if (onPersistRemoveBgModalMove) {
              selectedRemoveBgModalIds.forEach(modalId => {
                const modalState = removeBgModalStates.find(m => m.id === modalId);
                if (modalState) Promise.resolve(onPersistRemoveBgModalMove(modalId, { x: modalState.x, y: modalState.y })).catch(console.error);
              });
            }
            ...
            if (onPersistEraseModalMove) {
              selectedEraseModalIds.forEach(modalId => {
                const modalState = eraseModalStates.find(m => m.id === modalId);
                if (modalState) Promise.resolve(onPersistEraseModalMove(modalId, { x: modalState.x, y: modalState.y })).catch(console.error);
              });
            }
            ...
            if (onPersistExpandModalMove) {
              selectedExpandModalIds.forEach(modalId => {
                const modalState = expandModalStates.find(m => m.id === modalId);
                if (modalState) Promise.resolve(onPersistExpandModalMove(modalId, { x: modalState.x, y: modalState.y })).catch(console.error);
              });
            }
            ...
            if (onPersistVectorizeModalMove) {
              selectedVectorizeModalIds.forEach(modalId => {
                const modalState = vectorizeModalStates.find(m => m.id === modalId);
                if (modalState) Promise.resolve(onPersistVectorizeModalMove(modalId, { x: modalState.x, y: modalState.y })).catch(console.error);
              });
            }
            ...
            if (onPersistStoryboardModalMove) {
              selectedStoryboardModalIds.forEach(modalId => {
                const modalState = storyboardModalStates.find(m => m.id === modalId);
                if (modalState) Promise.resolve(onPersistStoryboardModalMove(modalId, { x: modalState.x || 0, y: modalState.y || 0 })).catch(console.error);
              });
            }

            // Persist final positions for scriptFrame modals
            if (onScriptFramePositionCommit) {
              selectedScriptFrameModalIds.forEach(modalId => {
                const modalState = scriptFrameModalStates.find(m => m.id === modalId);
                if (modalState) onScriptFramePositionCommit(modalId, modalState.x, modalState.y);
              });
            }

            // Persist final positions for sceneFrame modals
            if (onSceneFramePositionCommit) {
              selectedSceneFrameModalIds.forEach(modalId => {
                const modalState = sceneFrameModalStates.find(m => m.id === modalId);
                if (modalState) onSceneFramePositionCommit(modalId, modalState.x, modalState.y);
              });
            }
          }}
        >
