...
import { existsNearby, findAvailablePositionNear, applyStageCursor, checkOverlap, findBlankSpace, focusOnComponent, INFINITE_CANVAS_SIZE, DOT_SPACING, DOT_SIZE, DOT_OPACITY, getClientRect } from '@/core/canvas/canvasHelpers';
import { generateScenesFromStory, queryCanvasPrompt, createStitchedReferenceImage } from '@/core/api/api';
import { generateSingleScenePrompt, ReferenceDetails } from '@/core/utils/generateStoryboardPrompt';
import { CanvasTextState, CompareModalState } from '@/modules/canvas-overlays/types';
...
interface CanvasProps {
  ...
  onPersistGroupCreate?: (group: import('@/core/types/groupContainer').GroupContainerState) => void | Promise<void>;
  onPersistGroupUpdate?: (id: string, updates: Partial<import('@/core/types/groupContainer').GroupContainerState>, prev?: import('@/core/types/groupContainer').GroupContainerState) => void | Promise<void>;
  onPersistGroupDelete?: (group: import('@/core/types/groupContainer').GroupContainerState) => void | Promise<void>;
  // Allow initial groups to be provided from parent snapshot hydration
  initialGroupContainerStates?: import('@/core/types/groupContainer').GroupContainerState[];
  // Compare plugin persistence callbacks
  compareModalStates?: import('@/modules/canvas-overlays/types').CompareModalState[];
  setCompareModalStates?: React.Dispatch<React.SetStateAction<import('@/modules/canvas-overlays/types').CompareModalState[]>>;
  ...
  onPersistCanvasTextCreate?: (modal: import('@/modules/canvas-overlays/types').CanvasTextState) => void | Promise<void>;
  onPersistCanvasTextMove?: (id: string, updates: Partial<import('@/modules/canvas-overlays/types').CanvasTextState>) => void | Promise<void>;
...
export const Canvas: React.FC<CanvasProps> = ({
  ...
  const [localCanvasTextStates, setLocalCanvasTextStates] = useState<Array<import('@/modules/canvas-overlays/types').CanvasTextState>>([]);
  ...
  const [scriptFrameModalStates, setScriptFrameModalStates] = useState<Array<import('@/modules/canvas-overlays/types').ScriptFrameModalState>>([]);
  ...
  const [storyWorldStates, setStoryWorldStates] = useState<Array<{ storyboardId: string; storyWorld: import('@/core/types/storyWorld').StoryWorld }>>([]);
  ...
  const handleCanvasTextUpdate = (id: string, updates: Partial<CanvasTextState>) => {
  ...
  const handleSmartSceneUpdate = async (
    ...
    try {
      ...
      const outlineByNumber: Record<number, import('@/core/types/storyWorld').StorySceneOutline> = {};
  ...
  const handleCreateGroup = () => {
    ...
    const newGroup: import('@/core/types/groupContainer').GroupContainerState = {
      id: `group-${Date.now()}`,
      type: 'group',
      children: [],
      x: bounds.x,
  ...
  const generateScenesFromStoryboardInternal = async (
    ...
    try {
      ...
      const { buildImagePromptForScene } = await import('@/modules/plugins/StoryboardPluginModal/imagePromptUtils');
      const { getReferenceImagesFromStoryWorld } = await import('@/modules/plugins/StoryboardPluginModal/mentionUtils');
  ...
  const handleDuplicateSceneFrame = (frameId: string) => {
    let duplicatedFrame: import('@/modules/canvas-overlays/types').SceneFrameModalState | null = null;
    setSceneFrameModalStates(prev => {
      ...
      const newFrame: import('@/modules/canvas-overlays/types').SceneFrameModalState = {
        ...source,
  ...
  const upsertStoryWorld = (storyboardId: string, storyWorld: import('@/core/types/storyWorld').StoryWorld) => {
  ...
  const getStoryWorld = (storyboardId: string): import('@/core/types/storyWorld').StoryWorld | undefined => {
  ...
  // Sync externalMultiangleCameraModals changes to internal state
  useEffect(() => {
    if (externalMultiangleCameraModals !== undefined && externalMultiangleCameraModals.length === 0) {
      console.log('[Canvas] Clearing multiangleCameraModalStates (external is empty)');
      setMultiangleCameraModalStates([]);
      return;
    }
    if (externalMultiangleCameraModals && externalMultiangleCameraModals.length > 0) {
      setMultiangleCameraModalStates(prev => {
        const externalIds = new Set(externalMultiangleCameraModals.map(m => m.id));
        const prevIds = new Set(prev.map(m => m.id));
        const idsMatch = externalIds.size === prevIds.size && [...externalIds].every(id => prevIds.has(id));
        if (idsMatch) {
          return prev.map(prevModal => {
            const externalModal = externalMultiangleCameraModals.find(m => m.id === prevModal.id);
            if (externalModal) {
              return {
                ...prevModal,
                ...externalModal,
        ...
        return externalMultiangleCameraModals.map(m => ({ ...m, isExpanded: false }));
      });
    }
  }, [JSON.stringify(externalMultiangleCameraModals || [])]);
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      // Check if wheel event corresponds to the ComponentCreationMenu
      if ((e.target as HTMLElement).closest('[data-component-menu]')) {
        return;
      }
  ...
  // Helper function to update viewport center
  const updateViewportCenter = useCallback((pos: { x: number; y: number }, currentScale: number) => {
  ...
  }, [onViewportChange, viewportSize.width, viewportSize.height]);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (pointerPos) {
        ...
        // Check multiangle camera modals
        multiangleCameraModalStates.forEach((modal) => {
          const dims = getComponentDimensions('multiangleCameraModal', modal.id);
          if (hasOverlap(modal.x, modal.y, dims.width, dims.height)) {
            selectedMultiangleCameraModalIdsList.push(modal.id);
          }
        });
        ...
        setSelectedVideoEditorModalIds(selectedVideoEditorModalIdsList);
        ...
        setSelectedMultiangleCameraModalIds(selectedMultiangleCameraModalIdsList);
      ...
      if (currentBox) {
        ...
        if (wasDragged) {
          ...
          selectedUpscaleModalIds.forEach(id => {
            const m = upscaleModalStates.find(mm => mm.id === id);
            if (m) {
          ...
          selectedMultiangleCameraModalIds.forEach(id => {
            const m = multiangleCameraModalStates.find(mm => mm.id === id);
            if (m) {
              const dims = getComponentDimensions('multiangleCameraModal', id);
              allSelectedComponents.push({
                x: m.x,
                y: m.y,
                width: dims.width,
          ...
          selectedRemoveBgModalIds.forEach(id => {
            const m = removeBgModalStates.find(mm => mm.id === id);
            if (m) {
          ...
          selectedEraseModalIds.forEach(id => {
            const m = eraseModalStates.find(mm => mm.id === id);
            if (m) {
          ...
          selectedExpandModalIds.forEach(id => {
            const m = expandModalStates.find(mm => mm.id === id);
            if (m) {
          ...
          selectedVectorizeModalIds.forEach(id => {
            const m = vectorizeModalStates.find(mm => mm.id === id);
            if (m) {
          ...
          selectedNextSceneModalIds.forEach(id => {
            const m = nextSceneModalStates.find(mm => mm.id === id);
            if (m) {
              const dims = getComponentDimensions('nextSceneModal', id);
              allSelectedComponents.push({
                x: m.x,
                y: m.y,
                width: dims.width,
          ...
          selectedStoryboardModalIds.forEach(id => {
            const m = storyboardModalStates.find(mm => mm.id === id);
            if (m) {
          ...
          selectedScriptFrameModalIds.forEach(id => {
            const m = scriptFrameModalStates.find(mm => mm.id === id);
            if (m) {
          ...
          selectedSceneFrameModalIds.forEach(id => {
            const m = sceneFrameModalStates.find(mm => mm.id === id);
            if (m) {
  ...
  const handleUpdateImageModalState = (modalId: string, updates: any) => {
    setImageModalStates(prev => prev.map(m => m.id === modalId ? { ...m, ...updates } : m));
    // Persist changes if handler is available
  ...
  const handleStageMouseUp = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (selectionRectCoords) {
      ...
      if (rectWidth >= 5 && rectHeight >= 5) {
        ...
        const newSelectedVideoEditorModalIds: string[] = isMultiSelect ? [...selectedVideoEditorModalIds] : [];
        ...
        videoEditorModalStates.forEach((modal) => {
          ...
          const modalWidth = 100;
          const modalHeight = 120; // 100px circle + ~20px for label
          if (checkIntersection({ x: modal.x, y: modal.y, width: modalWidth, height: modalHeight })) {
        ...
        if (newSelectedVideoEditorModalIds.length > 0) {
          setSelectedVideoEditorModalId(newSelectedVideoEditorModalIds[0]);
        }
    ...
    if (selectionBox) {
      ...
      if (newSelectedVideoEditorModalIds.length > 0 || !isMultiSelect) {
        setSelectedVideoEditorModalIds(newSelectedVideoEditorModalIds);
        setSelectedVideoEditorModalId(newSelectedVideoEditorModalIds.length > 0 ? newSelectedVideoEditorModalIds[0] : null);
      }
      ...
      if (newSelectedMultiangleCameraModalIds.length > 0 || !isMultiSelect) {
        setSelectedMultiangleCameraModalIds(newSelectedMultiangleCameraModalIds);
        setSelectedMultiangleCameraModalId(newSelectedMultiangleCameraModalIds.length > 0 ? newSelectedMultiangleCameraModalIds[0] : null);
      }
    ...
    if (clickedOnEmpty && !isSelecting && !selectionBox) {
      ...
      const hasSelections = selectedImageIndex !== null ||
        ...
        selectedMultiangleCameraModalIds.length > 0 ||
  ...
  const moveSelectedItems = (dx: number, dy: number, excludeType?: string, excludeId?: string | number) => {
    ...
    moveModalGroup(selectedMultiangleCameraModalIds, multiangleCameraModalStates, onPersistMultiangleCameraModalMove, 'multiangle-camera');
  ...
  const handleMultiangleCameraModalMove = createMoveWrapper('multiangle-camera', selectedMultiangleCameraModalIds, multiangleCameraModalStates, onPersistMultiangleCameraModalMove);
      ...
      if (mediaData) {
        ...
        if (data && data.type === 'plugin' && data.plugin) {
          ...
          if (stageRef.current && containerRef.current) {
            ...
            } else if (data.plugin.id === 'multiangle-camera' && onPersistMultiangleCameraModalCreate) {
  ...
  // NextScene persistence handlers
  const handlePersistNextSceneModalCreate = (modal: any) => {
    setNextSceneModalStates(prev => [...prev, modal]);
    if (onPersistNextSceneModalCreate) {
      Promise.resolve(onPersistNextSceneModalCreate(modal)).catch(console.error);
    }
  };

  const handlePersistNextSceneModalMove = (id: string, updates: any) => {
    setNextSceneModalStates(prev => prev.map(m => m.id === id ? { ...m, ...updates } : m));
    if (onPersistNextSceneModalMove) {
      Promise.resolve(onPersistNextSceneModalMove(id, updates)).catch(console.error);
    }
  };

  const handlePersistNextSceneModalDelete = (id: string) => {
    setNextSceneModalStates(prev => prev.filter(m => m.id !== id));
    if (onPersistNextSceneModalDelete) {
      Promise.resolve(onPersistNextSceneModalDelete(id)).catch(console.error);
    }
    if (selectedNextSceneModalId === id) {
      setSelectedNextSceneModalId(null);
    }
          ...
          // Handle download for modals
            ...
            if (urlToDownload) {
              try {
                ...
                const { downloadFile, generateDownloadFilename } = await import('@/core/api/downloadUtils');
