...
interface BackgroundTypeDropdownProps {
  selectedBackgroundType: string;
  scale: number;
  isOpen: boolean;
  onToggle: () => void;
  onSelect: (backgroundType: string) => void;
}

export const BackgroundTypeDropdown: React.FC<BackgroundTypeDropdownProps> = ({
      ...
      {isOpen && (
        ...
        >
          {BACKGROUND_TYPES.map((backgroundType) => (
            ...
            >
              <span style={{ whiteSpace: 'nowrap' }}>{backgroundType}</span>
            </div>
          ))}
        </div>
      )}
