...
export function buildSnapshotElements(
  ...
  connectorsOverride?: Connector[]
): Record<string, any> { // eslint-disable-line @typescript-eslint/no-explicit-any
  const elements: Record<string, any> = {}; // eslint-disable-line @typescript-eslint/no-explicit-any

  // Build connection map keyed by source element id
  const connectionsBySource: Record<string, Partial<Connector>[]> = {};
  ...
  state.images.forEach((img, idx) => {
    const id = img.elementId || `element-${idx}`;
    if (!img.elementId) {
  ...
  state.imageGenerators.forEach((g) => {
    ...
    const metaObj = {
      ...
      sourceImageUrl: g.sourceImageUrl || null, // CRITICAL: Save sourceImageUrl to snapshot
  ...
  state.videoGenerators.forEach((g) => {
    const connections = connectionsBySource[g.id]?.length ? connectionsBySource[g.id] : undefined;
    const metaObj = {
      generatedVideoUrl: g.generatedVideoUrl || null,
      frameWidth: g.frameWidth,
      frameHeight: g.frameHeight,
      model: g.model,
      frame: g.frame,
  ...
  state.musicGenerators.forEach((g) => {
    const connections = connectionsBySource[g.id]?.length ? connectionsBySource[g.id] : undefined;
    const metaObj = {
      generatedMusicUrl: g.generatedMusicUrl || null,
      frameWidth: g.frameWidth,
      frameHeight: g.frameHeight,
      model: g.model,
      frame: g.frame,
  ...
  state.storyboardGenerators.forEach((modal) => {
    ...
    if (modal.characterNamesMap) {
      const characterMap: Record<string, string> = {};
      Object.entries(modal.characterNamesMap).forEach(([indexStr, name]) => {
        ...
        if (name && characterConnections[index]) {
          ...
          if (!imageUrl) {
            ...
            if (imageGen) {
              imageUrl = imageGen.generatedImageUrl ||
                imageGen.sourceImageUrl ||
                imageGen.generatedImageUrls?.[0] ||
                imageGen.url;
            }
          }
          ...
          if (!imageUrl) {
            const image = state.images.find(img => img.elementId === imageId);
            imageUrl = image?.url || image?.firebaseUrl;
          }
    ...
    if (modal.backgroundNamesMap) {
      const backgroundMap: Record<string, string> = {};
      Object.entries(modal.backgroundNamesMap).forEach(([indexStr, name]) => {
        const index = parseInt(indexStr, 10);
    ...
    if (modal.propsNamesMap) {
      const propsMap: Record<string, string> = {};
      Object.entries(modal.propsNamesMap).forEach(([indexStr, name]) => {
        const index = parseInt(indexStr, 10);
        ...
        if (name && propsConnections[index]) {
          ...
          if (imageUrl) {
            ...
            console.warn(`[buildSnapshotElements] ⚠️ No image URL found for prop "${name}" (imageId: ${imageId})`, {
              ...
              imageFound: !!state.images.find(img => img.elementId === imageId),
      ...
      if (Object.keys(propsMap).length > 0) {
        ...
        console.warn(`[buildSnapshotElements] ⚠️ No props mapped for ${modal.id}`, {
          propsNamesMap: modal.propsNamesMap,
          propsConnectionsCount: propsConnections.length,
        });
      }
    }
    ...
    if (Object.keys(namedImages).length > 0) {
      ...
      console.warn(`[buildSnapshotElements] ⚠️ No namedImages built for storyboard ${modal.id}`, {
        hasCharacterNamesMap: !!modal.characterNamesMap,
        hasBackgroundNamesMap: !!modal.backgroundNamesMap,
        hasPropsNamesMap: !!modal.propsNamesMap,
    ...
    const metaObj = {
      ...
      characterNamesMap: modal.characterNamesMap || {},
      propsNamesMap: modal.propsNamesMap || {},
      backgroundNamesMap: modal.backgroundNamesMap || {},
      stitchedImageUrl: modal.stitchedImageUrl || undefined,
      ...(Object.keys(namedImages).length > 0 ? { namedImages } : {}),
      ...(connections ? { connections } : {})
    };
  ...
  state.sceneFrameGenerators.forEach((modal) => {
    ...
    const metaObj = {
      ...
      characterIds: modal.characterIds || undefined,
      locationId: modal.locationId || undefined,
      mood: modal.mood || undefined,
      characterNames: modal.characterNames || undefined,
      locationName: modal.locationName || undefined,
      ...(connections ? { connections } : {})
    };
    elements[modal.id] = {
      id: modal.id,
